...
def h2_up_core(html_doc):
    ...
    # Empirically added no MOF words
            ...
            if MOF_Crit > 2:
                ...
                if not words[j][0].islower() or words[j][0:3] == 'sod' or words[j][0:3] == 'rho' or words[j][0:3] == 'bio' or \
    ...
    bold_tag = ['1','1′','2','3','4','5','6','7','7′','8','(1)','(2)','(3)','1a','1b','1c','2a','2b',
                '2c','3a','3b','3c','a','b','c','d','1 a','2 a','2 b','A','B','C','D','1 d','I','II']
    bold_tag_candidate = []
    bold_MOF = []
            ...
            while out == 0:
                ...
                if bold_index != -1 and bold_index != 0 and bold_index != len(bold_sentence):
                    if bold_sentence[bold_index-1].isdigit() or bold_sentence[bold_index-1].isalpha():
                        pass_crit += 1
                    try: #omert
                        if bold_index + len(i.string) <= len(bold_sentence) and \
                           bold_sentence[bold_index+len(i.string)].isdigit() or \
                           bold_sentence[bold_index+len(i.string)].isalpha():
                            pass_crit += 1
                    except:#omert
                        pass#omert:
                if pass_crit == 2:
            ...
            for k in range(len(bold_sentence)):
                ...
                if store == 0 and bold_sentence[k].find(i.string) != -1:
                    ...
                    if k != 0 and k != len(bold_sentence) - 1:
                        for l in range(len(MOF_in_Paper)):
                            if bold_sentence[k-1] == 'MOF' or bold_sentence[k-1] == '(MOF' or \
                               bold_sentence[k-1] == 'Compound' or bold_sentence[k-1] == '(Compound':
    ...
    Pore_table = re.compile(
        '^\w?PV\s?|^V\W{0,3}T\W?|^V\W{0,3}p$|^V\W{0,3}p\W+\W+|^V\W{0,3}meso\W?|\
        ^V\W{0,3}micro\W?|^V\W{0,3}total\W?|^V\W{0,3}pore\W?|Pore\s{0,3}Volume', re.IGNORECASE)
    Pore = re.compile(
        'Volume\W?\(cm3\s?g\)|mesopore\W?volume|micropore\W?volume|pore\W?volume|\
        \W+PV\s?|V\W?T|V\W?P|V\W?meso|V\W?micro|V\W?total|V\W?pore|effective\W?free\W?volume|permanent\W?porosity', re.IGNORECASE)
    ...
    range_prob = re.compile('\s?range\s?[-]?\s?of\s\d+\s?\W?\s?\d+|\s?excess\s?of\s?\d+|\
                            from\s?\d+\s?to\s?\d+\s?|from\s?\d+|over\s?\d+|\d+\s?~\s?\d+|\d+\s?–\s?\d+|as\s?high\s?as\s?\d+|\
                            ∼\d+|[<>]\d+|\s?exceeding\s?\d+|\s?higher\s?than\s?\d+|\s?larger\s?than\s?\d+|theoretical\s?value\s?of\s?\d+', re.I)
...
# html_dir = "/home/omert/Downloads/mof_lib/1_5000_files/acs_lib"
